         ...
         create: function (req, res) {
            ...
            //  debug('Inside create..............req.params = ' + JSON.stringify(req.params.all()));
             ...
             return Model.create(payload).then(function (record) {
                 ...
                 //Move to it's own helper:
                 if(req._sails.hooks.pubsub){
                     if(req.isSocket){
                         Model.subscribe(req, record);
                         Model.introduce(record);
                     }
                     Model.publishCreate(record.toJSON(), !req.options.mirror && req);
                 }
             ...
             return Model.findByIdFromRequest(req).then(function (record) {
                 if (record) {
                     record.destroy().then(function (record) {
                        ...
                        if(req._sails.hooks.pubsub){
                              Model.publishDestroy(id, !req.options.mirror && req, {previous:record});
                              if(req.isSocket){
